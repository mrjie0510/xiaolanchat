...
diyici = [f"不要戳小蓝好不啦", f"戳了我你就是新的机器人拉，小蓝洗衣服去了哦",f"阿啦啦，啦啦啦啦啦，小蓝被戳拉，哦呵呵"]
dierci = [f"不要总是戳啦", f"又戳啊，不太好吧",f"小蓝又被戳啦！"]
disanci = [f"再戳报警了哦", f"群主在哪里，管理在哪里，有人一直戳小蓝啊",f"谁来管管啊，小蓝不喜欢一直被戳戳戳啊！"]
        ...
        if ids.startswith("group"):
            ...
            if group_id in Config.used_in_group:
                ...
                if values['notice_type'] == 'notify' and values['sub_type'] == 'poke' and str(
                    ...
                    if user_id in Config.super_uid:
                        await chat_notice.send("如果是你的话，想戳多少次都可以哦~" + MessageSegment.image(img_path + '爱心.jpg'))
                    # 如果不在响应cd
                        ...
                        if randint(0, 99) < Config.p_poke_response:
                            ...
                            # 清空ban列表
                            poke_ban_list[group_id] == {}
                            await chat_notice.send(
                                random.choice(diyici) + MessageSegment.image(img_path + '叉腰.jpg'))
                    else:
                        if response == 0:
                            if randint(0, 99) < Config.p_poke_response:
                                response += 1
                                poke_ban_list[group_id][user_id] = 1
                                await chat_notice.send(random.choice(dierci) + MessageSegment.image(img_path + '嘟嘴.jpg'))
                        elif response == 1:
                            if randint(0, 99) < Config.p_poke_response:
                                response += 1
                                if user_id in poke_ban_list[group_id]:
                                    ...
                                    poke_ban_list[group_id][user_id] = 1
                                await chat_notice.send(
                                    random.choice(disanci) + MessageSegment.image(
                                        img_path + '嘟嘴.jpg'))
                        elif response == 2:
                            if randint(0, 99) < Config.p_poke_response:
                                response += 1
                                ...
                                try:
                                    ...
                                    pass
                                await chat_notice.send("mdzz！给爷似吧！" + MessageSegment.image(img_path + '贴纸.jpg'))
                        else:
                            # 如果戳过了，那么每戳一次就把禁言时间翻倍
                            if user_id in poke_ban_list[group_id]:
                                poke_ban_list[group_id][user_id] += 1
